	...
	public void onVariablesAvailable(UnaryOperator<IVariable> lookup) {
		...
		if(cr!=null) {
			...
			if(allow)
				...
				conds=cr.getDeny();
			if(conds!=null)
				allowence=conds.stream().map(e->e.getTranslation(GameTranslation.get())).map(StringTextComponent::new).collect(Collectors.toList());
			if(allowence!=null&&!allowence.isEmpty()) {
				if(allow)
					allowence.add(0,new TranslationTextComponent("recipe.thermopolium.allow"));
				else
					allowence.add(0,new TranslationTextComponent("recipe.thermopolium.deny"));
			}
		}
	}
