...
public class THRecipeProvider extends RecipeProvider {
	...
	protected void registerRecipes(@Nonnull Consumer<IFinishedRecipe> out) {
		...
		cook("egg_tongsui").special().med().base().type(water).and().require().half().of(eggs).and().any().of(sugar)
				.and().not().any().of(meats).plus(seafood).plus(vegetables).plus(mrl("wolfberries")).and().then()
				.finish(out);
		...
		cook("goji_tongsui").special().med().base().type(water).and().require().mainly().of(sugar).and().any()
				.of(mrl("wolfberries")).and().not().any().of(meats).plus(seafood).plus(vegetables).and().then()
				.finish(out);
		cook("ukha").special().med().base().tag(anyWater).and().require().half().of(fish).plus(roots).and().not().any()
				.of(meats).plus(coreals).and().then().finish(out);
		cook("goulash").special().high().base().type(stock).and().require().mainly().of(ftag("raw_beef")).and().any()
				.of(vegetables).and().not().any().of(seafood).plus(coreals).and().then().finish(out);
		cook("okroshka").special().high().require().half().of(vegetables).plus(meats).and().any().of(vegetables).and()
				.any().of(mrl("ice")).and().then().finish(out);
		cook("nettle_soup").special().med().require().half().of(mrl("fern")).and().not().any().of(seafood).plus(meats)
				.plus(coreals).and().then().finish(out);
	}
